...
namespace AmplifyShaderEditor
{
    ...
    public class BillboardOpHelper
    {
        ...
        public static readonly string[] BillboardCylindricalInstructions =
        {
            "//Calculate new billboard vertex position and normal",
            "float3 upCamVec = float3( 0, 1, 0 )"
        };

        public static readonly string[] BillboardSphericalInstructions =
        {
            "//Calculate new billboard vertex position and normal",
        ...
        public static readonly string[] BillboardCommonInstructions =
        {
        ...
        public static readonly string[] BillboardRotDependent =
        {
        ...
        public static readonly string[] BillboardRotIndependent =
        {
            "{0}.x *= length( unity_ObjectToWorld._m00_m10_m20 )",
        ...
        public static readonly string[] BillboardHDRotDependent =
        {
            "//This unfortunately must be made to take non-uniform scaling into account",
            "//Transform to world coords, apply rotation and transform back to local",
        ...
        public static readonly string[] BillboardHDRotIndependent =
        {
            "{0}.x *= length( GetObjectToWorldMatrix()._m00_m10_m20 )",
        ...
        [SerializeField] private bool m_isBillboard = false;

        [SerializeField] private BillboardType m_billboardType = BillboardType.Cylindrical;

        [SerializeField] private bool m_rotationIndependent = false;
        ...
        public static void FillDataCollector(ref MasterNodeDataCollector dataCollector, BillboardType billboardType,
            bool rotationIndependent, string vertexPosValue, string vertexNormalValue, bool vertexIsFloat3)
        {
            switch (billboardType)
            {
                case BillboardType.Cylindrical:
                {
                    for (int i = 0; i < BillboardCylindricalInstructions.Length; i++)
                    {
                        dataCollector.AddVertexInstruction(
                            BillboardCylindricalInstructions[i] + (dataCollector.IsTemplate ? ";" : string.Empty), -1,
                            true);
                    }
                }
                    break;
                ...
                case BillboardType.Spherical:
                {
                    ...
                    {
                        dataCollector.AddVertexInstruction(
                            BillboardSphericalInstructions[i] + (dataCollector.IsTemplate ? ";" : string.Empty), -1,
                            true);
                    }
                }
                    break;
            ...
            {
                string value = (i == 3)
                    ? string.Format(BillboardCommonInstructions[i], vertexNormalValue)
                    : BillboardCommonInstructions[i];
            ...
            if (rotationIndependent)
            {
                for (int i = 0; i < BillboardRotIndependent.Length; i++)
                {
                    ...
                    if (dataCollector.IsTemplate && dataCollector.TemplateDataCollectorInstance.CurrentSRPType !=
                        TemplateSRPType.BuiltIn)
                    {
                        value = (i != 5)
                            ? string.Format(BillboardHDRotIndependent[i], vertexPosValue)
                            : BillboardHDRotIndependent[i];
                    }
                    else
                    {
                        value = (i != 5)
                            ? string.Format(BillboardRotIndependent[i], vertexPosValue)
                            : BillboardRotIndependent[i];
                    }

                    dataCollector.AddVertexInstruction(value + (dataCollector.IsTemplate ? ";" : string.Empty), -1,
                        true);
                }
            }
            else
            {
                string vertexPosConverted =
                    vertexIsFloat3 ? string.Format("float4({0},0)", vertexPosValue) : vertexPosValue;
                for (int i = 0; i < BillboardRotDependent.Length; i++)
                {
                    ...
                    {
                        value = (i > 1)
                            ? string.Format(BillboardHDRotDependent[i], vertexPosValue, vertexPosConverted,
                                (vertexIsFloat3 ? ".xyz" : string.Empty))
                            : BillboardHDRotDependent[i];
                    }
                    else
                    {
                        value = (i > 1)
                            ? string.Format(BillboardRotDependent[i], vertexPosValue, vertexPosConverted,
                                (vertexIsFloat3 ? ".xyz" : string.Empty))
                            : BillboardRotDependent[i];
                    }
        ...
        public static string[] GetMultilineInstructions(BillboardType billboardType, bool rotationIndependent,
            string vertexPosValue, string vertexNormalValue)
        {
            // This method is only used on Surface ... no HD variation is needed
            ...
            else
            {
                ...
                {
                    string value = (i > 1)
                        ? string.Format(BillboardRotDependent[i], vertexPosValue)
                        : BillboardRotDependent[i];
                    body.Add(value);
                }
            }
