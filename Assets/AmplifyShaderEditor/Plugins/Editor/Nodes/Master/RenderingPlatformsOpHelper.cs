...
namespace AmplifyShaderEditor
{
    ...
    public class RenderingPlatformOpHelper
    {
        ...
        [SerializeField] private bool[] m_renderingPlatformValues;
        ...
        public void Draw(ParentNode owner)
        {
            ...
            NodeUtils.DrawPropertyGroup(ref value, RenderingPlatformsStr, () =>
            {
                for (int i = 0; i < m_renderingPlatformValues.Length; i++)
                {
                    m_renderingPlatformValues[i] = owner.EditorGUILayoutToggleLeft(RenderingPlatformsInfo[i].Label,
                        m_renderingPlatformValues[i]);
                }
            });
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            else
            {
                ...
                if (count > 0)
                {
                    RenderPlatforms firstPlatform =
                        (RenderPlatforms)Enum.Parse(typeof(RenderPlatforms), nodeParams[index++]);
