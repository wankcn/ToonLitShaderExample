...
{
    [Serializable]
    [NodeAttributes("Switch by Pipeline", "Functions", "Executes branch according to current pipeline",
    ...
    public sealed class FunctionSwitchByPipeline : ParentNode
    {
        ...
        void UpdateOutputPort()
        {
            switch (UIUtils.CurrentWindow.OutsideGraph.CurrentSRPType)
            {
                ...
                {
                    InputPort port = UIUtils.CurrentWindow.OutsideGraph.IsStandardSurface
                        ? GetInputPortByUniqueId(0)
                        : GetInputPortByUniqueId(3);
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 16303)
            {
                ...
                {
                    UIUtils.SetConnection(UniqueId, 3, standardPort.GetConnection().NodeId,
                        standardPort.GetConnection().PortId);
                }
            }
        }
    }
