...
{
    [Serializable]
    [NodeAttributes("Function Node", "Functions", "Function Node", KeyCode.None, false, 0, int.MaxValue,
        typeof(AmplifyShaderFunction))]
    public class FunctionNode : ParentNode
    {
        [SerializeField] private AmplifyShaderFunction m_function;

        [SerializeField] private ParentGraph m_functionGraph;

        [SerializeField] private int m_functionGraphId = -1;

        [SerializeField] private List<FunctionInput> m_allFunctionInputs;
        private Dictionary<int, FunctionInput> m_allFunctionInputsDict = new Dictionary<int, FunctionInput>();

        [SerializeField] private List<FunctionOutput> m_allFunctionOutputs;
        private Dictionary<int, FunctionOutput> m_allFunctionOutputsDict = new Dictionary<int, FunctionOutput>();

        [SerializeField] private List<FunctionSwitch> m_allFunctionSwitches;
        private Dictionary<int, FunctionSwitch> m_allFunctionSwitchesDict = new Dictionary<int, FunctionSwitch>();

        [SerializeField] private ReordenatorNode m_reordenator;

        [SerializeField] private string m_filename;

        [SerializeField] private string m_headerTitle = string.Empty;

        [SerializeField] private int m_orderIndex;

        [SerializeField] private string m_functionCheckSum;

        [SerializeField] private string m_functionGUID = string.Empty;
        ...
        [SerializeField] private ParentGraph m_outsideGraph = null;

        [SerializeField] private FunctionOutput m_mainPreviewNode;
        ...
        public void CommonInit(AmplifyShaderFunction function, int uniqueId)
        {
            ...
            AmplifyShaderEditorWindow.LoadFromMeta(ref m_functionGraph, ContainerGraph.ParentWindow.ContextMenuInstance,
                Function.FunctionInfo);
            //m_functionCheckSum = LastLine( m_function.FunctionInfo );
            m_functionCheckSum =
                AssetDatabase.GetAssetDependencyHash(AssetDatabase.GetAssetPath(m_function)).ToString();
            ...
            for (int i = 0; i < inputCount; i++)
            {
                ...
                {
                    AddInputPort(m_allFunctionInputs[i].SelectedInputType, false, m_allFunctionInputs[i].InputName, -1,
                        MasterNodePortCategory.Fragment, m_allFunctionInputs[i].UniqueId);
                }
            ...
            for (int i = 0; i < outputCount; i++)
            {
                ...
                {
                    AddOutputPort(m_allFunctionOutputs[i].AutoOutputType, m_allFunctionOutputs[i].OutputName,
                        m_allFunctionOutputs[i].UniqueId);
                }
        ...
        {
            List<TemplateMultiPassMasterNode> nodes =
                ContainerGraph.ParentWindow.OutsideGraph.GetMultiPassMasterNodes(lod);
            int count = nodes.Count;
            for (int i = 0; i < count; i++)
            {
                nodes[i].PassModule.AdditionalDirectives
                    .AddShaderFunctionItems(OutputId, Function.AdditionalDirectives.DirectivesList);
            }
        }
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (surface != null)
            {
                ...
                //}
                surface.AdditionalDirectives.AddShaderFunctionItems(OutputId,
            ...
            else
            {
                ...
                {
                    for (int lod = -1;
                         lod < ContainerGraph.ParentWindow.OutsideGraph.LodMultiPassMasternodes.Count;
                         lod++)
                    {
                        AddShaderFunctionDirectivesInternal(lod);
                    }
            ...
            if (ReadOptionsHelper.Length > 2)
            {
                for (int i = 1; i < ReadOptionsHelper.Length; i += 2)
                {
                    ...
                    for (int j = 0; j < m_allFunctionSwitches.Count; j++)
                    {
                        ...
                        {
                            m_allFunctionSwitches[j].SetCurrentSelectedInput(optionValue,
                                m_allFunctionSwitches[j].GetCurrentSelectedInput());
                            break;
                        }
                    }
                }
        ...
        void InputPortSwitchRestriction(WirePort port)
        {
            switch (port.DataType)
            {
                ...
                {
                    port.CreatePortRestrictions(WirePortDataType.FLOAT, WirePortDataType.FLOAT2,
                        WirePortDataType.FLOAT3, WirePortDataType.FLOAT4, WirePortDataType.COLOR, WirePortDataType.INT,
                ...
                {
                    port.CreatePortRestrictions(WirePortDataType.FLOAT3x3, WirePortDataType.FLOAT4x4,
                        WirePortDataType.OBJECT);
                }
                    break;
                ...
                {
                    port.CreatePortRestrictions(WirePortDataType.SAMPLER1D, WirePortDataType.SAMPLER2D,
        ...
        void OutputPortSwitchRestriction(WirePort port)
        {
            switch (port.DataType)
            {
                ...
                case WirePortDataType.FLOAT4x4:
                {
                    port.AddPortForbiddenTypes(WirePortDataType.SAMPLER1D, WirePortDataType.SAMPLER2D,
                        WirePortDataType.SAMPLER3D, WirePortDataType.SAMPLERCUBE, WirePortDataType.SAMPLER2DARRAY);
                }
                    break;
                case WirePortDataType.SAMPLER1D:
            ...
            bool activateNode = true)
        {
            ...
            FunctionInput functionInput =
                m_refreshIdsRequired ? m_allFunctionInputs[portId] : GetFunctionInputByUniqueId(portId);
            ...
            string name, WirePortDataType type)
        {
            ...
            FunctionInput functionInput = m_refreshIdsRequired
                ? m_allFunctionInputs[inputPortId]
                : GetFunctionInputByUniqueId(inputPortId);
            functionInput.PreviewIsDirty = true;
            if (functionInput.AutoCast)
            {
                InputPort inputPort = m_refreshIdsRequired
                    ? m_inputPorts[inputPortId]
                    : GetInputPortByUniqueId(inputPortId);
                inputPort.MatchPortToConnection();
        ...
        public override void DrawProperties()
        {
            ...
            if (inputCount > 0)
            {
                for (int i = 0; i < inputCount; i++)
                {
                    if (m_inputPorts[i].Available && m_inputPorts[i].ValidInternalData &&
                        !m_inputPorts[i].IsConnected &&
                        m_inputPorts[i]
                            .AutoDrawInternalData /*&& ( m_inputPorts[ i ].AutoDrawInternalData || ( m_autoDrawInternalPortData && m_useInternalPortData ) )*/
            ...
            if (drawInternalDataUI)
                NodeUtils.DrawPropertyGroup(ref m_internalDataFoldout, Constants.InternalDataLabelStr, () =>
                {
                    ...
                    {
                        if (m_inputPorts[i].ValidInternalData && !m_inputPorts[i].IsConnected &&
                            m_inputPorts[i].Visible && m_inputPorts[i].AutoDrawInternalData)
                        {
        ...
        public override void Destroy()
        {
            ...
            if (ContainerGraph.ParentWindow.OutsideGraph.CurrentStandardSurface != null)
            {
                ...
                //}
                ContainerGraph.ParentWindow.OutsideGraph.CurrentStandardSurface.AdditionalDirectives
                    .RemoveShaderFunctionItems(OutputId /*, m_directives */);
            }
            else
            {
                if (ContainerGraph.ParentWindow.OutsideGraph.MultiPassMasterNodes.Count > 0)
        ...
        public bool CheckForChanges(bool forceCheck = false, bool forceChange = false)
        {
            if ((ContainerGraph.ParentWindow.CheckFunctions || forceCheck || forceChange) && m_function != null)
            {
                ...
                string newCheckSum = AssetDatabase.GetAssetDependencyHash(AssetDatabase.GetAssetPath(m_function))
                    .ToString();
        ...
        public void DuplicateMe()
        {
            ...
            for (int i = 0; i < m_allFunctionSwitches.Count; i++)
            {
                allOptions += "," + m_allFunctionSwitches[i].UniqueId + "," +
                              m_allFunctionSwitches[i].GetCurrentSelectedInput();
            }
            ...
            for (int i = 0; i < m_outputPorts.Count; i++)
            {
                if (m_outputPorts[i].IsConnected)
                {
                    ...
                    if (newNode.OutputPorts != null && newOutputPort != null)
                    {
                        ...
                        {
                            ContainerGraph.CreateConnection(m_outputPorts[i].ExternalReferences[j].NodeId,
                                m_outputPorts[i].ExternalReferences[j].PortId, newOutputPort.NodeId,
                                newOutputPort.PortId);
                        }
                    }
                }
                //else
            ...
            for (int i = 0; i < m_inputPorts.Count; i++)
            {
                if (m_inputPorts[i].IsConnected)
                {
                    ...
                    {
                        ContainerGraph.CreateConnection(newInputPort.NodeId, newInputPort.PortId,
                            m_inputPorts[i].ExternalReferences[0].NodeId, m_inputPorts[i].ExternalReferences[0].PortId);
                    }
                }
            }
        ...
        private string FunctionNodeOnPortGeneration(ref MasterNodeDataCollector dataCollector, int index,
            ParentGraph graph)
        {
            ParentGraph cachedGraph = ContainerGraph.ParentWindow.CustomGraph;
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(ref nodeInfo,
                AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(m_function)));
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            AmplifyShaderFunction loaded =
                AssetDatabase.LoadAssetAtPath<AmplifyShaderFunction>(AssetDatabase.GUIDToAssetPath(m_functionGUID));
            if (loaded != null)
            {
            ...
            else
            {
                ...
                if (guids.Length > 0)
                {
                    ...
                    loaded = AssetDatabase.LoadAssetAtPath<AmplifyShaderFunction>(
                        AssetDatabase.GUIDToAssetPath(sfGuid));
                    ...
                    else
                    {
                        ...
                        UIUtils.ShowMessage(UniqueId,
                            string.Format("Error loading {0} shader function from project folder", m_filename),
                ...
                else
                {
                    ...
                    UIUtils.ShowMessage(UniqueId,
                        string.Format("Missing {0} shader function on project folder", m_filename),
                        MessageSeverity.Error);
                }
            }
        ...
        private void ConfigureInputportsAfterRead()
        {
            if (InputPorts != null)
            {
                ...
                for (int i = 0; i < inputCount; i++)
                {
                    InputPorts[i].ChangeProperties(m_allFunctionInputs[i].InputName,
                        m_allFunctionInputs[i].SelectedInputType, false);
                }
            }
            ...
            if (OutputPorts != null)
            {
                ...
                for (int i = 0; i < outputCount; i++)
                {
                    OutputPorts[i].ChangeProperties(m_allFunctionOutputs[i].OutputName,
                        m_allFunctionOutputs[i].AutoOutputType, false);
                }
            }
        }
